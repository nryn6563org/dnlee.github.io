...
<div>
    <section id="openBankingAgree" class="agreePage">
        <div class="titleInStep">
            <h1>약관에 동의해주세요</h1>
            <p>선택하신 금융사의 출금/조회 약관에 동의해 주세요.</p>
        </div>
    ...
    <div class="buttonArea">
        <button typw="button" class="h50 pointBlue"
            ...
            @click="$router.push({ name: 'openBankingJoinResult' })">다음</button>
    </div>
...
const pageInfo = reactive({
    bankLists: new Array(0), // 동의서 화면용 배열
    isCheckedAll: false,
    userEmail: '',
    agreeList: new Array(0)
})
// 컴포넌트 실행 분기
...
const preRun = () => {
    if(props.joinInfo.accInfo.length !== 0) {
        props.joinInfo.accInfo.forEach((arr) => {
            if(pageInfo.bankLists.indexOf(arr.bankCode) === -1) {
                pageInfo.bankLists.push(arr.bankCode)
            }
        })
        console.log(pageInfo.bankLists)
        pageInfo.bankLists.forEach((arr) => {
            const addModel = {}
            const addObj = {
                text: $codeToBank(arr).korName + ' 출금/조회 약관',
                viewPage: arr,
                isChecked: false, // 실제 하위항목이 있을 시 화면상에서는 사용되지 않는다.
                mustReg: true
            }
            Object.assign(addModel, addObj)
            pageInfo.agreeList.push(addModel)
        })
    }
}
