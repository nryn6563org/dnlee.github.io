...
<div>
    ...
    <section id="buyELB">
        ...
        <ul class="roundBox">
            ...
            <li>
                <span>주문가능금액</span>
                <p>{{$priceToCommas(props.pageInfo.itemList.possibleAmount)}}원</p>
            </li>
        </ul>
        <div class="inputBox">
            <label class="price">
    ...
    <div class="bottomBtnArea align01">
        ...
        <button typw="button" class="h50 pointBlue"
            @click="$router.push({ name: 'buyELBConfirm' })">다음</button>
    </div>
...
console.log('d', props.pageInfo.selectTabMenu)
const pageInfo = reactive({
    isPassed: true, // 비밀번호 입력여부
...
const runComp = (comps, directInput) => {
    ...
    if(comps === selectMyAccListInTypes) {
        ...
        document.getElementById('buyELB').style.overflow = 'hidden'
        const moveTop = focusElement.getBoundingClientRect().top - 200
        setTimeout(() => {
            document.getElementById('buyELB').scrollBy({
                top: moveTop,
                left: 0,
                behavior: 'smooth'
            })
...
const closeComp = (returnVals) => {
    ...
    if(returnVals !== false) {
        if (comps === regPrice) {
            document.getElementById('buyELB').style.overflow = 'auto'
